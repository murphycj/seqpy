...
def main(args):
    ...
    rscript = folder + '/' + args.name + '.R'
    rlog = folder + '/' + args.name + '.log'
    filehandle = open(rscript, 'w')

    filehandle.write('#\n')
    filehandle.write('# Comparison ' + args.name + '\n')
    ...
    filehandle.write('plot_and_save_results(res, \"' + args.name + '\")\n\n')
...
parser.add_argument('--counts',type=str,help='csv file containing RNA-seq count (using human gene symbols)',required=True)
...
parser.add_argument('--name',type=str,help='Name of DESeq2 comparison',required=True)
